<template>
  <v-layout>
    ...
    <v-flex xs12 sm12 md4 xl2 class="ml-2">
      <v-card>
        <v-card-text>
          <h3 v-if="!isNew" class="font-weight-bold mb-2">Last Modified on</h3>
          <span class="mb-2" v-if="!isNew">{{ formatDate(updated_at) }} by <u>{{ updaterName }}</u></span>
          <v-select
            v-model="status_id"
            :items="statuses"
            item-value="id"
        ...
        <v-card-actions>
          <v-layout text-xs-center row wrap>
              ...
              <v-flex xs12>
                <a v-if="!isNew" href="#/move-to-trash" @click.prevent="deleteItem">Move to trash</a>
              </v-flex>
            </v-layout>
        </v-card-actions>
      </v-card>
