...
describe('components/supplier/SupplierFleets', () => {
  ...
  describe('Props & Computed', () => {
    ...
    test('storeStatuses', async () => {
      const wrapper = shallowMount(Component)
      expect(wrapper.vm.storeStatuses).toHaveLength(0)

      wrapper.setProps({
        storeStatuses: FIXTURE_STORE_STATUSES
      })
      await wrapper.vm.$nextTick()
      expect(wrapper.vm.storeStatuses).toMatchObject(FIXTURE_STORE_STATUSES)
    })
  ...
  describe('Methods', () => {
    ...
    test('changeStatus(value, item)', () => {
      ...
      expect(emitted[0][1]).toMatchObject(value)
    })
    describe('manageMultiple(action, items, value)', () => {
      ...
      test('when action = status', () => {
        ...
        expect(emitted[0][1]).toMatchObject(items)
        expect(emitted[0][2]).toMatchObject(2)
      })
    })
  })
})
