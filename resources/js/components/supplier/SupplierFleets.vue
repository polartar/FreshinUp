<template>
  <v-card>
    ...
    <v-layout row wrap class="px-4 py-2">
      <v-flex xs12 sm6 md3 v-for="(stat, statIndex) in statusStats" :key="statIndex">
        <store-status-stat
          :label="stat.label"
          :value="stat.value"
          :color="stat.color"
        ...
        <v-flex xs12>
          ...
          >
            ...
            <template v-slot:item-inner-name="{ item }">
              <a class="font-weight-bold" href="#view-item" @click.prevent="viewItem(item)">{{ item.name }}</a> <br/>
              {{ get(item, 'type.name') }}
            </template>
